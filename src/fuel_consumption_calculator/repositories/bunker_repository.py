...
class BunkerRepository:
    ...
    def save_receiving_tank_plan(self, plan: PlannedBunker, rows: list[BunkerReceivingTankPlan], incoming: BunkerIncomingFuelSnapshot) -> None:
        ...
        with self._database.connect() as connection:
            ...
            connection.execute("INSERT INTO bunker_incoming_fuel_snapshots (vessel_id,sequence_number,port_snapshot,arrival_snapshot,fuel_batch_id,density_15_kg_m3,manual_vcf,incoming_temperature_c,vcf_mode,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(vessel_id,sequence_number,port_snapshot,arrival_snapshot) DO UPDATE SET fuel_batch_id=excluded.fuel_batch_id,density_15_kg_m3=excluded.density_15_kg_m3,manual_vcf=excluded.manual_vcf,incoming_temperature_c=excluded.incoming_temperature_c,vcf_mode=excluded.vcf_mode,updated_at=excluded.updated_at", (*key, incoming.fuel_batch_id, incoming.density_15_kg_m3, incoming.manual_vcf, incoming.incoming_temperature_c, incoming.vcf_mode, timestamp, timestamp))
    ...
    def load_incoming_fuel_snapshot(self, plan: PlannedBunker) -> BunkerIncomingFuelSnapshot:
        with self._database.connect() as connection:
            row = connection.execute("SELECT fuel_batch_id,density_15_kg_m3,manual_vcf,incoming_temperature_c,vcf_mode FROM bunker_incoming_fuel_snapshots WHERE vessel_id=? AND sequence_number=? AND port_snapshot=? AND arrival_snapshot=?", (plan.vessel_id,plan.sequence_number,plan.port_snapshot,plan.arrival_snapshot)).fetchone()
        return BunkerIncomingFuelSnapshot(None,None,None) if row is None else BunkerIncomingFuelSnapshot(row["fuel_batch_id"], row["density_15_kg_m3"], row["manual_vcf"], row["incoming_temperature_c"], row["vcf_mode"])
